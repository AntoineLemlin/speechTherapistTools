...
const Modal = ({onSave, children}: {onSave: FormEventHandler<HTMLFormElement>,children: ReactNode}) => {
  const openModalRef = useRef<HTMLDialogElement>(null);
  return (
    <>
      {/* You can open the modal using document.getElementById('ID').showModal() method */}
    <button className="btn" onClick={()=>openModalRef.current?.showModal()}><Cog6ToothIcon className="w-6 h-6" /> </button>
